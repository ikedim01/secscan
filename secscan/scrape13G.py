...
__all__ = ['default13GDir', 'getSec13NshAndPctFromText2', 'addNshAndPct', 'cusipChecksum', 'tryCusipFix',
           'monthNameToIso', 'getMonthPatStr', 'parseEventDate', 'getFivePctXmlInfo', 'parse13GD', 'scraper13G',
           'reOPTS', 'aggregatePatStr', 'percentOfClassPatStr', 'typeOfRepPatStr', 'form13PiecesPat1',
           'form13PiecesPat2', 'form13PiecesPat3', 'nSharesPatStr', 'nPctBarePatStr', 'nPctWithPctPatStr',
           'nshAndPctPat1Pref', 'form13NshAndPctPats1', 'form13NshAndPctPat2', 'form13NshAndPctPat3', 'purposePat',
           'strictCusipPatStr', 'cusipPatStr', 'cusipNumberPatStr', 'cusipSearchPats', 'spaceDashPat', 'monthNames',
           'monthAbbrevStrs', 'monthPatStr', 'monthDayPatStr', 'possCommaPatStr', 'yearPatStr', 'dateOfEventPatStr',
           ...
           'dateOfEventIsoRevPat1', 'dateOfEventIsoPat2', 'dateOfEventIsoRevPat2', 'whitespacePat', 'xmlEventDatePat',
           'dateStrMMDDPat', 'xmlReportingPersonPat', 'xmlClassPctPat', 'xmlAggregateNSharesPat', 'xmlCusipPat',
           'xmlPurposePat', 'updateCik13GDPos', 'cikSymStr', 'calcBonusMap']
...
#
# Scrape old text format filings (SC 13G, SC 13D):
#
...
def tryCusipFix(cusip) :
    ...
    if len(cusip) == 8 :
        if cusipChecksum('0'+cusip[:7]) == cusip[7] :
            cusip = '0'+cusip
            print('adding leading 0 to CUSIP', cusip, end=' ')
        else :
...
#
# Scrape new XML format filings (SCHEDULE 13G, SCHEDULE 13D):
#

xmlEventDatePat = re.compile('(?:eventdate|dateofevent).*',re.IGNORECASE)
dateStrMMDDPat = re.compile(r"(\d\d)[/\-](\d\d)[/\-](\d\d\d\d)$")
xmlReportingPersonPat = re.compile('.*reportingperson(?:detail|info).*',re.IGNORECASE)
xmlClassPctPat = re.compile('(?:classpercent|percent.*class)',re.IGNORECASE)
xmlAggregateNSharesPat = re.compile('.*aggregate(?:numberofshares|amountowned)',re.IGNORECASE)
xmlCusipPat = re.compile('issuercusip',re.IGNORECASE)
xmlPurposePat = re.compile('.*(?:purpose.*trans|trans.*purpose).*',re.IGNORECASE)

def getFivePctXmlInfo(info, xmlInfo, formType, textLimit) :
    eventDateTag = xmlInfo.find(xmlEventDatePat)
    if eventDateTag is None :
        print('NO EVENT DATE!', end=' ')
    else :
        dStr = utils.getCombSoupText(eventDateTag).strip().upper()
        m = dateStrMMDDPat.match(dStr)
        if m is None :
            print('EVENT DATE NOT IN MM/DD/YYYY FORMAT!', end=' ')
            # info['eventDate'] = dStr
        else :
            info['eventDate'] = m.group(3)+'-'+m.group(1)+'-'+m.group(2)
    positions = []
    for posTag in xmlInfo.find_all(xmlReportingPersonPat) :
        aggregateNSharesTag = posTag.find(xmlAggregateNSharesPat)
        classPctTag = posTag.find(xmlClassPctPat)
        if aggregateNSharesTag is None :
            print('AGGREGATE NSHARES NOT FOUND IN', posTag, end=' ')
        elif classPctTag is None :
            print('CLASS PERCENT NOT FOUND IN', posTag, end=' ')
        else :
            positions.append((utils.getCombSoupText(aggregateNSharesTag).strip(),
                              utils.getCombSoupText(classPctTag).strip()))
    info['positions'] = positions
    cusipTag = xmlInfo.find(xmlCusipPat)
    if cusipTag is None :
        print('no CUSIP found!', end=' ')
    else :
        info['cusip'] = tryCusipFix(utils.getCombSoupText(cusipTag).strip()).upper()
    if dailyList.isInFormClass('FIVEPCTACTIVE', formType) :
        purposeTag = xmlInfo.find(xmlPurposePat)
        if purposeTag is None :
            print('no purpose!', end=' ')
        else :
            info['purpose'] = utils.getCombSoupText(purposeTag).strip()[:textLimit]
...
def parse13GD(accNo, formType=None, info=None, textLimit=basicInfo.defaultTextLimit, debug=False) :
    ...
    if formType is None :
        formType = links[0][2]
    if len(links)>=2 and links[1][-1].lower().endswith('.xml') :
        # scrape using new XML format:
        xmlInfo = utils.downloadSecUrl(links[1][-1], toFormat='soup')
        getFivePctXmlInfo(info, xmlInfo, formType, textLimit)
    else :
        # scrape using old text format:
