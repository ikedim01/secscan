...
__all__ = ['default13FDir', 'findChildEndingWith', 'findChildSeries', 'getRowInfo', 'parse13FHoldings', 'scraper13F',
           'callOptPat', 'putOptPat', 'condenseHoldings', 'holdingsReportList', 'condenseHoldingsOld',
...
@utils.delegates(condenseHoldings)
def holdingsReportList(holdings, cusipNames={}, **kwargs) :
    """
    Converts a list of of stock and option holdings as parsed from the 13F:
        [(cusip, name, value, title, count, putCall), ... ]
    ...
    Uses condenseHoldings to condense, sort, and filter the returned list.
    """
    condensedH = condenseHoldings(holdings, **kwargs)
    if condensedH is None :
        return []
    cusipToNameInForm = dict((cusip,name)
                       for cusip, name, value, shType, nShares, putCall in holdings)
    return [(cusip, cusipNames.get(cusip,cusipToNameInForm[cusip]), val, f'{frac:.2%}')
            for (cusip, val, frac) in condensedH]

def condenseHoldingsOld(holdings, minFrac=0.0, maxFrac=1.0,
                     pctFormat=False, includeName=False, cusipNames={},
                     minStocksPerInv=None, maxStocksPerInv=None,
                     minTopNFrac=None, minTopN=10, minAUM=None,
                     allCusipCounter=None, all13FHoldingsMap=None, forCik=None) :
    """
    Converts a list of of stock and option holdings as parsed from the 13F:
        [(cusip, name, value, title, count, putCall), ... ]
    ...
    If minStocksPerInv, maxStocksPerInv, minTopNFrac or minAUM are specified, returns None
    for lists with too few stocks, too many stocks, too small a fraction in the
    top minTopN stocks, or too small a total value.
...
@utils.delegates(condenseHoldings)
def getHoldingsMap(scraped13F, period, **kwargs) :
    """
    ...
    Uses condenseHoldings to condense, sort, and filter the returned holdings and,
    optionally (if allCusipCounter and all13FHoldingsMaps are supplied), to count
    all investors per stock, and to compute a separate map from CIK to a sorted list
    containing all stock holdings for each CIK:
        all13FHoldingsMap[cik] = [(cusip, val, frac) ... ]
    """
    print('calculating 13F holdings map for',period)
    minTopN = kwargs.get('minTopN')
    for v,msg in [('minFrac','min stock fraction of portfolio'),
                  ('maxFrac','max stock fraction of portfolio'),
                  ('minTopNFrac',f'min fraction of portfolio in top {minTopN} positions'),
                  ('minAUM','min AUM (total portfolio value)')] :
        if kwargs.get(v) is not None :
            print(msg, kwargs.get(v))
    #
    ...
    for cik, cik13FList in cikTo13Fs.items() :
        ...
        posList = condenseHoldings(combHoldings, forCik=cik, **kwargs)
